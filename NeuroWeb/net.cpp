...
vector<double> MultilayerPerceptron::forward_propagation(const vector<double>& input, vector<double>& hidden_output)
{
	...
	for (int i = 0; i < hidden_output.size(); i++) hidden_output[i] = modrelu(hidden_output[i]);
	vector<double> output = matrix_multiply_sum(weights_secondL, hidden_output, hidden_size, output_size, bias_secondL);
	for (int i = 0; i < output.size(); i++) output[i] = modrelu(output[i]);
	return output;
}
...
void MultilayerPerceptron::backpropagation_updating_weights(const vector<double>& output, const vector<double>& target, double learning_rate, const vector<double>& input)
{
	...
	for (int i = 0; i < output_size; i++) {
		output_errors.push_back((target[i] - output[i]) * modrelu_derivative(output[i]));
	}
...
vector<double> MultilayerPerceptron::matrix_multiply(const vector < vector < double>>& A, const vector<double>& B, int input_neuro, int output_neuro, const vector<double>& input)
{
	...
	for (int i = 0; i < input_neuro; ++i) {
		...
		temp *= modrelu_derivative(input[i]);
		C.push_back(temp);
	}
...
void MultilayerPerceptron::train(const vector < vector < double>>&
	inputs, const vector < vector < double>>& targets, int epochs, double learning_rate, string ans)
{
...
bool read_test(vector<int>& config, vector < vector < double>>& inputs_other, vector < vector < double>>& targets_other) {
	...
	void task_test(int v, double L, string ans, vector < vector < double>> & inputs_other, vector < vector < double>> & targets_other, MultilayerPerceptron & MLP) {
		...
		while (z != "end") {
			...
			for (int i = 0; i < output.size(); i++) cout << round(output[i]) << " ";
			cout << endl;
			cout << "Чтобы выйти из режима тестирования введите end\n"; cin >> z;
		}
	}
		...
		vector < vector < double>> & targets_and, vector < vector < double>> & targets_xor, MultilayerPerceptron & mlp, string ans)
	{
		...
		if (a == "OR") {
			...
			cout << "Сейчас Вы должны ввести свои данные для тестирования нейросети\nЧтобы выйти из режима тестирования введите 2\n";
			cin >> q;
			while (!cin || cin.peek() != '\n' || !(q == 0 || q == 1 || q == 2)) {
				clean();
				cout << "Введите числа (только 1 и 0), к которым примените оператор OR\nЧтобы выйти из режима тестирования введите 2\n"; cin >> q;
			}
			if (q == 2) return;
			cin >> w;
			while (!cin || cin.peek() != '\n' || !(w == 0 || w == 1 || w == 2)) {
			...
			while (!cin || cin.peek() != '\n' || !(q == 0 || q == 1 || q == 2)) {
				clean();
				cout << "Введите числа (только 1 и 0), к которым примените оператор AND\nЧтобы выйти из режима тестирования введите 2\n"; cin >> q;
			}
			if (q == 2) return;
			cin >> w;
			while (!cin || cin.peek() != '\n' || !(w == 0 || w == 1 || w == 2)) {
			...
			while (!cin || cin.peek() != '\n' || !(q == 0 || q == 1 || q == 2)) {
				clean();
				cout << "Введите числа (только 1 и 0), к которым примените оператор XOR\nЧтобы выйти из режима тестирования введите 2\n"; cin >> q;
			}
			if (q == 2) return;
			cin >> w;
			while (!cin || cin.peek() != '\n' || !(w == 0 || w == 1 || w == 2)) {
